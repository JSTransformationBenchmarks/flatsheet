(async function () {
  ...
  var templates = {
    table: await fs_readFilePromise(__dirname + '/views/table.html', 'utf8'),
    sheetDetails: await fs_readFilePromise(__dirname + '/views/sheet-details.html', 'utf8'),
    modal: Handlebars.compile(await fs_readFilePromise(__dirname + '/views/modal.html', 'utf8')),
    userList: Handlebars.compile(await fs_readFilePromise(__dirname + '/views/user-list.html', 'utf8')),
    editLongText: Handlebars.compile(await fs_readFilePromise(__dirname + '/views/editor-long-text.html', 'utf8')),
    settings: Handlebars.compile(await fs_readFilePromise(__dirname + '/views/sheet-settings.html', 'utf8'))
  };
  io.on('connect', function () {
    ...
    io.on('update-users', function (userlist) {
      usersEl.innerHTML = templates.userList({
        users: userlist
      });
    });
  });
  ...
  io.on('change', function (change, rows, sort) {
    remoteChange = true;
    if (sort) editor.forceUpdate(rows);else editor.set(change);
    remoteChange = false;
  });
  ...
  io.on('disconnect', function () {//console.log('disconnection.');
  });
  ...
  on(document.body, '#show-csv', 'click', function (e) {
    var csv_file = new CSV(editor.getRows(), {
      header: true
    }).encode();
  ...
  on(document.body, '#settings', 'click', function (e) {
    e.preventDefault();
    var sheet = sheetDetails.get(); // Get an array of all (filtered) accounts objects in the site

    flatsheet.accounts.list(function (err, accounts) {
      if (err) return console.log("sheet.settings: err:", err); // Create an associative array to easily access account data
      ...
      function appendProperties(key) {
        return {
          key: key,
          username: accountsDict[key].username,
          color: accountsDict[key].color
        };
      }
      ...
      dom.add(document.body, domify(modal)); // convert accounts array to an array of account usernames (currently usernames are used as keys)

      var suggestedEditors = accounts.filter(editorsFilter).map(function (account) {
      ...
      var suggestedOwners = accounts.filter(sheetOwnersFilter).map(function (account) {
        return account.username;
      });
  ...
  on(document.body, '.delete-sheet-permission', 'click', function (e) {
    ...
    var array = btn.id.split("_"),
        // button id's: 'testusername-key-user` and 'testusername-key-owner'
    username = array[0],
        key = array[1],
        changeType = array[2];
  ...
  on(document.body, 'thead .destroy', 'click', function (e) {
    var id;
    if (elClass(e.target).has('destroy')) id = e.target.id;else if (elClass(e.target).has('destroy-icon')) id = closest(e.target, '.destroy').id;
  ...
  on(document.body, '.delete-row', 'click', function (e) {
    var btn;
    if (elClass(e.target).has('delete-row')) btn = e.target;else if (elClass(e.target).has('destroy-icon')) btn = closest(e.target, '.delete-row');
  ...
  on(document.body, '.expand-editor', 'click', function (e) {
    ...
    var text = cell.value;
    var modal = templates.modal({
      content: templates.editLongText({
        text: text,
        id: id
      })
    });
