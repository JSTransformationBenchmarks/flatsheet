(async function () {
  ...
  var flatsheet = await require('../lib/index.js')(config, ready);
  ...
  function updateSheetsStream(sheetStream) {
    var self = this;
    sheetStream.on('data', function (sheet) {
  ...
  var usernameToUuidPermissionsHelper = function (usernameDict, cb) {
    ...
    each(usernameList, function (username, _, next) {
      ...
      if (!UUID_REGEX.test(username)) {
        console.log("update.migrateSheet: getting key from username:", username); // Get uuid from username

        flatsheet.accountsIndexes.getKeyFromUsername(username, function (err, account) {
          if (err) return console.log("update.usernameToUuid: cannot get key from " + "username:", err);
  ...
  var sendPasswordResetEmail = function (account, next) {
    ...
    flatsheet.resets.put(token, opts, function (err) {
      ...
      var data = {
        url: flatsheet.site.url + '/accounts/acceptReset?username=' + account.value.username + '&key=' + account.value.key + '&token=' + token,
      ...
      flatsheet.email.sendMail(message, function (err, info) {
        if (err) return console.log(err);
        console.log("Update.sendResetEmail: Email sent to account email:", account.value.email); // info is `{ message: 'success'}`
  ...
  var deleteSessionsResetsAndIndexes = function () {
    ...
    flatsheet.db.createReadStream().on('data', function (data) {
      // Delete all session rows
      if (sessionRegex.test(data.key)) {
        console.log("deleting session:", data); // delete row
