...
async function Server(opts, cb) {
  ...
  if (await fs_existsPromise(envFilePath)) {
    var envFile = await fs_readFilePromise(envFilePath);
  ...
  */
  ...
  mkdirp(this.dataDir, function (err) {
    if (err) console.error(err);else initdb(self, cb);
  });
  /*
  * Email
  */
...
Server.prototype.listen = function (port, cb) {
  ...
  var staticFiles = st({
    path: this.staticFiles,
    url: this.staticFileUrl
  });
  ...
  fs.readdir(this.viewsDir, function (err, files) {
    for (const [arrayIteratorIndex, file] of files.entries()) {
      if (file === 'layout.html') {
        ...
        Handlebars.registerPartial('layout', await fs_readFilePromise(filepath, 'utf8'));
      } else await self.addView(file);
    }
  });
};
/*
...
Server.prototype.addView = async function (file, viewsDir) {
  var dir = viewsDir || this.viewsDir;
  return this.views[file.split('.')[0]] = await this.compileView(dir + file);
};
/*
* compile a view
*/
...
Server.prototype.compileView = async function (filepath) {
  var template = Handlebars.compile(await fs_readFilePromise(filepath, 'utf8'));
  return template;
};
/*
* Override views
  ...
  fs.readdir(this.viewsOverrideDir, function (err, files) {
    for (const [arrayIteratorIndex, file] of files.entries()) {
      if (file === 'layout.html') {
        ...
        Handlebars.registerPartial('layout', await fs_readFilePromise(filepath, 'utf8'));
      } else await self.addView(file, self.viewsOverrideDir);
    }
  });
};
/*
